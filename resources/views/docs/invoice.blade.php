<!DOCTYPE html>
<html lang="en">
<?php
use App\Helpers\DataHelper;
use App\Models\Due;

?>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Bukti Pembayaran Siswa</title>
    <style>
        .container {
            width: 100%;
            margin: 0 auto;
            font-family: Arial, sans-serif;
        }

        table {
            width: 100%;
            border-collapse: collapse;
        }

        th,
        td {
            border: 0px solid black;
            padding: 5px;
            text-align: left;
            font-size: 12px;
        }

        th {
            background-color: #eee;
        }

        p {
            /* line-height: 0.2; */
            margin-bottom: 3px;
            margin-top: 0;
            font-size: 12px;
        }

        .grand-total {
            font-weight: bold;
            font-size: 14px;
        }
    </style>
</head>

<body>
    <div class="container">
        <table style="width: 100%; border: 0">
            <tr>
                <td style="width: 20%;">
                    <img src="data:image/jpeg;base64," width="80px" alt="">
                </td>
                <td style="width: 60%;">
                    <center>
                        <h1 style="margin-bottom: 5px;">SEKOLAH BAPTIS PALEMBANG</h1>
                        <p>Jl. Jendral Sudirman No. 490 Km. 2,5 Sekip Jaya</p>
                        <p>Telp. (0711) 311-238 | Web: www.sekolahbaptispalembang.com</p>
                    </center>
                </td>
                <td style="width: 20%;"></td>
            </tr>
        </table>

        <center>
            <hr style="margin-bottom: 10px;">
            <b style="margin: 0px;">Tagihan Akademik Siswa</b>
            <hr style="margin-top: 10px;">
        </center>

        <table style="width: 100%; border: 0">
            <tr>
                <td style="width: 50%; vertical-align: top; padding: 0;">
                    <table>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>NIS</b></td>
                            <td style="width: 65%; padding: 2px;">{{ $student->nis }}</td>
                        </tr>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>Nama Siswa</b></td>
                            <td style="width: 65%; padding: 2px;">{{ $student->name }}</td>
                        </tr>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>Kelas</b></td>
                            <td style="width: 65%; padding: 2px;">{{ $student->backtrack_current_classroom_name }}</td>
                        </tr>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>Semester</b></td>
                            <td style="width: 65%; padding: 2px;">Ganjil 2024/2025</td>
                        </tr>
                    </table>
                </td>
                <td style="width: 50%; vertical-align: top; padding: 0;">
                    <table>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>Status</b></td>
                            <td style="width: 65%; padding: 2px; color: #ff0000;">Belum Lunas</td>
                        </tr>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>Nomor Rek Maspion</b></td>
                            <td style="width: 65%; padding: 2px;">01391{{ $va_number }}</td>
                        </tr>
                        <tr>
                            <td style="width: 35%; padding: 2px;"><b>Nomor VA BCA</b></td>
                            <td style="width: 65%; padding: 2px;">{{ isset($bca_va_number) ? $bca_va_number : "-" }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
        </table>

        <hr>

        <table>
            <tr>
                <th>No.</th>
                <th>Keterangan Pembayaran</th>
                <th style="text-align: right;">Jumlah (Rp.)</th>
            </tr>
            @php
                $i = 1;
                $grand_total = 0;
            @endphp
            @foreach ($invoice_details as $invoice_detail)
                <tr>
                    <td>{{ $i++ }}</td>
                    <td>{{ $invoice_detail->due_name }} - {{ DataHelper::get_month_name(intval($invoice_detail->payment_for_month) - 1) }} {{ $invoice_detail->payment_for_year }}</td>
                    <td style="text-align: right;">{{ number_format($invoice_detail->price) }}</td>
                </tr>
            @php
                $grand_total += $invoice_detail->price;
            @endphp
            @endforeach
            <tr>
                <td></td>
                <td class="grand-total">Grand Total</td>
                <td class="grand-total" style="text-align: right;">{{ number_format($grand_total) }}</td>
            </tr>
        </table>
        <br>
        <?php
            $data_helper = new DataHelper;
        ?>
        <p><b>Terbilang</b> : <i><?= ucwords($data_helper->idr_currency_factory($grand_total)) ?></i></p>

        <hr>
        <?php
        $student_current_grade = $student->backtrack_class_grade;
        $dues = Due::where("name", "like", "Ekskul%")->orWhere("name", "like", "Les%")->where("backtrack_class_grade", "=", $student_current_grade)->get();
        $i = 1;
        ?>
        @if (count($dues) > 0)
        {{--<b style="font-size: 12px;">Daftar Biaya Ekskul & Les</b>--}}
        <br>

        @if ($student->backtrack_class_grade == 00)
            <b style="margin: 0; font-size: 10px;">Les TK</b>
            <ul style="font-size: 10px;">
                <li>Calistung : 100rb</li>
                <li>Mandarin : 50rb</li>
                <li>Inggris : 50rb</li>
            </ul>

            <b style="margin: 0; font-size: 10px;">Ekskul TK</b>
            <ul style="font-size: 10px;">
                <li>Coloring : 60rb</li>
                <li>Modern Dance : 50rb</li>
                <li>Musik : 50rb</li>
            </ul>
        @endif

        @if ($student->backtrack_class_grade == 01)
            <b style="margin: 0; font-size: 10px;">Les SD</b>
            <ul style="font-size: 10px;">
                <li>Tematik : 125rb</li>
                <li>Mandarin : 45rb</li>
                <li>Inggris : 45rb</li>
            </ul>

            <b style="margin: 0; font-size: 10px;">Ekskul SD : 75rb</b>
            <ul style="font-size: 10px;">
                <li>Band</li>
                <li>Paduan Suara</li>
                <li>Sempoa</li>
                <li>Robotik</li>
                <li>Karate</li>
                <li>Futsal</li>
                <li>Komputer</li>
                <li>English Club</li>
            </ul>
        @endif

        <!-- <table style="font-size: 10px;">
            <tr>
                <th>No.</th>
                <th>Nama</th>
                <th style="text-align: right;">Biaya (Rp.)</th>
            </tr>
            @foreach ($dues as $due)
            <tr>
                <td>{{ $i++ }}</td>
                <td>{{ $due->name }}</td>
                <td style="text-align: right;">{{ number_format($due->price) }}</td>
            </tr>
            @endforeach
        </table> -->
        
        {{--<ul style="font-size: 10px !important;">
            @foreach ($dues as $due)
            <li>
                {{ $due->name }}: {{ number_format($due->price) }}
            </li>
            @endforeach
        </ul>--}}
        @endif

        <p style="font-size: 10px;">
            <b>Catatan:</b>
        <ul style="font-size: 10px;">
            <li>Pembayaran harus sesuai dengan ketentuan yang telah ditetapkan.</li>
            <li>Pembayaran dilakukan <b>sebelum tanggal 10</b> setiap bulannya</li>
            <li>Untuk informasi lebih lanjut mengenai pembayaran silahkan menghubungi nomor whatsapp official Sekolah Baptis Palembang (0811-7888-409)</li>
        </ul>
        </p>
    </div>
</body>

</html>
